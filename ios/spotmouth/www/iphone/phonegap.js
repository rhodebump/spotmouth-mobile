...
Camera.prototype.getPicture = function(successCallback, errorCallback, options) {
   // alert('getPicture');
    if(!Camera.successCallback) 
    {
     //   alert('getPicture2');
        navigator.camera.successCallback = successCallback;
        navigator.camera.errorCallback = errorCallback;
	    PhoneGap.exec("Camera.getPicture", "navigator.camera.reportSuccess", "navigator.camera.reportFailure", options);
    }
}

Camera.prototype.reportSuccess = function(imageData) {
  //alert('reportSuccess');
                                                    var cb = navigator.camera.successCallback;
                                                    navigator.camera.successCallback = null;
                                                    navigator.camera.errorCallback = null;
                                                    if(cb) { 
                                                       // alert('calling successcallback');
                                                    cb(imageData); 
                                                    } else {
                                                    // alert('not calling successcallback');
                                                     }
    }
                                                   
Camera.prototype.reportFailure = function(errorString) {
  //alert('reportFailure');
                                                    var cb = navigator.camera.errorCallback;
                                                    navigator.camera.successCallback = null;
                                                    navigator.camera.errorCallback = null;
                                                    if(cb) { 
                                                        //alert('calling error callback');
                                                        cb(errorString); 
                                                    }else {
                                                       // alert('not callign errro callback');
                                                    }
}
...
{
    alert("getCurrentPosition 1");
    var referenceTime = 0;
	
	if(this.lastError != null)
	{
        alert("getCurrentPosition 2");
		if(typeof(errorCallback) == 'function')
		{
            alert("getCurrentPosition 2a");          
			errorCallback.call(null,this.lastError);
		}
		this.stop();
		return;
	}
    alert("getCurrentPosition 3");
	this.start(options);

    var timeout = 20000; // defaults
    var interval = 500;
    alert("getCurrentPosition 4zz");

    alert("getCurrentPosition 5ccc");
        if (successCallback == null) {
        alert("successCallback is null");
        }  else {
         alert("successCallback is not null");
        }
                    
    alert("x=" + typeof(options));    
    if (typeof(options) == 'object' && options.interval) {
        alert("getCurrentPosition 4c");
        interval = options.interval;
    }
            alert("getCurrentPosition 5c");
            

        
    if (errorCallback == null) {
        alert("errorCallback is null");
    } else {
            alert("errorCallback is not null");
    }
    
    alert("typeof successCallback " + typeof(successCallback));

    alert("typeof errorCallback " + typeof(errorCallback));
    
    
    if (typeof(successCallback) != 'function') {
        successCallback = function() {};
        alert("getCurrentPostion 4a");
    }
    if (typeof(errorCallback) != 'function') {
        errorCallback = function() {};
        alert("getCurrentPostion 4b");
    }
        
    alert("getCurrentPosition 5");
    var dis = this;
    var delay = 0;
